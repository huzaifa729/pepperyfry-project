import React from 'react'
import './End.css'
function End() {
    return (
        <div>
            <div className="photose">

                <div className="dui">
                    <span className="pen">LOOKING FOR SOMETHING?</span>
                    <span className="pin">Everything Is Right Here</span>
                </div>


                <div className="split">
                    <div className="dab">
                        <div className="mo">
                            <a href="/"><img src="https://ii2.pepperfry.com/media/wysiwyg/banners/hp-consulting-sol-find-a-studio-img-18082020.jpg" alt="co" /></a>
                        </div>
                        <div className="bind">
                            <span className="three">FIND A STUDIO</span>
                            <span className="four">Walk In to your Nearest Studio Pepperfry</span>
                        </div>
                    </div>


                    <div className="dab">
                        <div className="mo">
                            <a href="/"><img src="https://ii3.pepperfry.com/media/wysiwyg/banners/hp-consulting-sol-by-on-phone-img-18082020.jpg" alt="co" /></a>
                        </div>
                        <div className="bind">
                            <span className="three">BUY ON PHONE</span>
                            <span className="four">Talk To Our Experts for a Free Consultation</span>
                        </div>
                    </div>


                    <div className="dab">
                        <div className="mo">
                            <a href="/"><img src="https://ii1.pepperfry.com/media/wysiwyg/banners/hp-consulting-sol-modular-solutions-img-18082020.jpg" alt="co" /></a>
                        </div>
                        <div className="bind">
                            <span className="three">MODULAR FURNITURE</span>
                            <span className="four">Get Customized Modular Kitchens and Wardrobe</span>
                        </div>
                    </div>
                </div>
            </div>

            <div className="new">

                <div className="dui">
                    <span className="pen">LOOKING FOR SOMETHING?</span>
                    <span className="pin">Everything Is Right Here</span>
                </div>

                <div className="imagine">
                    <div className="buks">
                        <div className="duk">
                           <a href="/"><img src="https://ii2.pepperfry.com/media/wysiwyg/banners/home-store-furniture-20198.jpg" alt="no" /></a>
                        </div>
                        <div className="row">
                            <ul>
                                <span className="ay">FURNITURE</span>
                                <li><a href="/">Sofas</a></li>
                                <li><a href="/">Seatings</a></li>
                                <li><a href="/">Chairs</a></li>
                                <li><a href="/">Tables</a></li>
                                <li><a href="/">Cabinetry</a></li>
                                <li><a href="/">Dining & Bar</a></li>
                                <li><a href="/">Beds</a></li>
                            </ul>
                        </div>
                    </div>


                    <div className="again">
                        <div className="bun">
                            <div className="duk">
                              <a href="/">  <img src="https://ii3.pepperfry.com/media/wysiwyg/banners/home-store-decor-20198.jpg" alt="no" /></a>
                            </div>
                      <div className="row">
                                 <ul>
                                    <span className="ay">HOME DECOR</span>
                                    <li><a href="/">Wall Acents</a></li>
                                    <li><a href="/">Wall Arts</a></li>
                                    <li><a href="/">Table Decor</a></li>
                                    <li><a href="/">Spiritual</a></li>
                                    <li><a href="/">Tableware</a></li>
                                    <li><a href="/">Home Garden</a></li>
                                    <li><a href="/">Home Services</a></li>
                                </ul>
                            </div>
                        </div>


                        <div className="bun">
                            <div className="duk">
                              <a href="/">  <img src="https://ii1.pepperfry.com/media/wysiwyg/banners/home-store-furnishings-2020.jpg" alt="no" /></a>
                            </div>
                            <div className="row">
                                    <ul>
                                    <span className="ay">FURNISHINGS</span>
                                    <li><a href="/">Bed Linen</a></li>
                                    <li><a href="/">Flooring</a></li>
                                    <li><a href="/">Curtains & Drapes</a></li>
                                    <li><a href="/">Bath Linen</a></li>
                                    <li><a href="/">Cushion and Covers</a></li>
                                    <li><a href="/">Table Linen</a></li>
                                    <li><a href="/">Essentials</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>




                    <div className="again">
                        <div className="bun">
                            <div className="duk">
                              <a href="/"> <img src="https://ii2.pepperfry.com/media/wysiwyg/banners/home-store-kids-20198.jpg" alt="no" /></a>
                            </div>
                     <div className="row">
                                 <ul>
                                    <span className="ay">KIDS ROOM</span>
                                    <li><a href="/">Beds</a></li>
                                    <li><a href="/">Study</a></li>
                                    <li><a href="/">Storage</a></li>
                                    <li><a href="/">Seating</a></li>
                                    <li><a href="/">Decor</a></li>
                                    <li><a href="/">Bedding</a></li>
                                    <li><a href="/">Furnishings</a></li>
                                </ul>
                            </div>
                        </div>


                        <div className="bun">
                            <div className="duk">
                              <a href="/"> <img src="https://ii3.pepperfry.com/media/wysiwyg/banners/home-store-lamps-20198.jpg" alt="no" /></a>
                            </div>
                            <div className="row">
                                  <ul>
                                    <span className="ay">LAMPS & LGHTS</span>
                                    <li><a href="/">Lamps</a></li>
                                    <li><a href="/">Ceiling Lights</a></li>
                                    <li><a href="/">Wall Lights</a></li>
                                    <li><a href="/">Festive Lights</a></li>
                                    <li><a href="/">Outdoor Lights</a></li>
                                   <li><a href="/">Smart Lights</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>

                    <div className="buks">
                        <div className="duk">
                           <a href="/"><img src="https://ii1.pepperfry.com/media/wysiwyg/banners/home-store-mattresses-2020.jpg" alt="no" /></a>
                        </div>
                        <div className="row">
                              <ul>
                                <span className="ay">MATTRESSES</span>
                                <li><a href="/">Mattresses</a></li>
                                <li><a href="/">Foldable Mattresses</a></li>
                                <li><a href="/">Baby Mattresses</a></li>
                                <li><a href="/">Mattress Protectors</a></li>
                                <li><a href="/">Mattress Toppers</a></li>
                                <li><a href="/">Pillows</a></li>
                                <li><a href="/">Bed Wedges</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    )
}

export default End
